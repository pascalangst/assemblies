...
#SBATCH --output=ccsmeth_4.out     #These are the STDOUT and STDERR files
#SBATCH --error=ccsmeth_4.err
...
ccsmeth call_freqb \
  --input_bam output.hifi.call_mods.modbam.pbmm2.bam \
  --ref GB-ELK1-1.busco.blast.self.fa \
  --output output.hifi.call_mods.modbam.pbmm2.freq \
  --threads 10 --sort --bed \
  --call_mode aggregate \
  --aggre_model ~/software/ccsmeth/models/model_ccsmeth_5mCpG_aggregate_attbigru_b11.v2p.ckpt
